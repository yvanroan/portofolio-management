...
//				add_tf.setBounds(270, 150, 350, 30);
		...
		vistable.add(new JScrollPane(vistab), BorderLayout.CENTER);
		...
		thumbnail.addTab(tabs[5], lsvisites);
		thumbnail.addTab(tabs[6], infoscl);
	...
	public void initComData (Commercial c) {
		...
		com_passf.setText(com.getPassword());
	}
	...
	public void actionPerformed (ActionEvent e) {
		...
		if (btnSource == info) {
			...
			for (Client cl: listcl) {
				Commercial clCom = new Commercial();
				clCom = comManager.find(cl.getIdCom());
				rows.addRow(new Object[] {cl.getIdClient(), cl.getName(), cl.getSurname(), cl.getAddress(), cl.getLocation(), cl.getIdCom(),
						clCom.getName(), clCom.getSurname()}
				); 
			}
		...
		if (btnSource == visite) {
			com = new Commercial();
			com.setIdCom(1);
			String[] title = {"IDclient", "Nom", "Prénom", "Adresse", "Localisation", "periode", "but"};
			Object[][] data = {};
			...
			for(Visit vs: listvis) {
				...
				rows.addRow(new Object[] {cl.getIdClient(), cl.getName(), cl.getSurname(), cl.getAddress(), cl.getLocation(), vs.getPeriod(),
						vs.getGoal()});
			}
		...
		if (btnSource == signup) {
			...
			confirm_pass = com_conpassf.getText(); phone = Integer.valueOf(com_phonef.getText().replaceAll("\\s", ""));
