...
public class VisitModel {
	...
	public List <Visit> findVisits(Commercial com, Client client) {
		try {
			...
			PreparedStatement ps = ConnectDB.getConnection().prepareStatement("SELECT cl.name, cl.surname, cm.name, cm.surname, period, goal"
					+ "  FROM client cl, commercial cm, visite v WHERE v.IDCom = ? and v.IDClient = ? and cm.IDCom = v.IDCom and cl.IDClient IN "
					+ "(SELECT cl.IDClient FROM client cl, commercial cm, visite v WHERE cl.IDCom = cm.IDCom and cl.IDClient = ?) and "
					+ "period <= ?");
	...
	public List <Visit> find(Commercial com) {
		try {
			listVisits = new ArrayList <Visit>();
			PreparedStatement ps = ConnectDB.getConnection().prepareStatement("SELECT * FROM visite WHERE idCom = ?");
			ps.setInt(1, com.getIdCom());
			ResultSet rs = ps.executeQuery();
			while(rs.next()) {
				Visit visit = new Visit();
	...
	public boolean add(int idcom, int idclient, String goal, String description, Timestamp period) {
		try {
			PreparedStatement ps = ConnectDB.getConnection().prepareStatement("INSERT INTO visite VALUES (?, ?, ?, ?, ?)");
			ps.setInt(1, idclient);
			ps.setInt(2, idcom);
			ps.setTimestamp(3, period);
			ps.setString(4, goal);
	...
	public boolean update(int idcom, int idclient, String goal, String description, Timestamp period) {
		try {
			PreparedStatement ps = ConnectDB.getConnection().prepareStatement("UPDATE visite SET "
